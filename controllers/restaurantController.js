...
const createRestaurant = async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Access denied. Admins only can create restaurants.' });
    }
...
const updateRestaurant = async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Access denied. Admins only can update restaurants.' });
    }
...
const deleteRestaurant = async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Access denied. Admins only can delete restaurants.' });
    }
